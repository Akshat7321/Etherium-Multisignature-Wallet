...
contract('Multisigwallet', (accounts) => {
	...
	it('Should create a wallet with 0 balance initially and 2 signers',function () {	
		return deployMultisig([accounts[0],accounts[1]],1).then(function(instance){
	...
	it("Adding new Users",function(){
		return deployMultisig([accounts[0], accounts[1]],1).then(function(instance){
			...
			return contractInstance.requestNewController(accounts[2],true,{
				from: accounts[0]
			});
	...
	it("Deleting existing user",function(){
		...
		.then(function(){
			return contractInstance.requestNewController(accounts[1],false,{
				from: accounts[0]
			});
		}).then(function(){
		 return contractInstance.forRequest({
		...
		return	contractInstance.proceedWithRequest	({
			...
			return contractInstance.transactionControllers(accounts[1])
		}).then(function(result){
